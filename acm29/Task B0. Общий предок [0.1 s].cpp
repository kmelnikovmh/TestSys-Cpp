...
int MAX_N_TZ = 50'001;
int MAX_DEEP_TZ = 16;           // 2 ^ 17 > 50'001
std::vector<std::vector<int>> tree(MAX_N_TZ);
std::vector<int> depth(MAX_N_TZ);
...
void dfs(int v, int father) {
    ...
    for (auto to : tree[v]) {
        depth[to] = depth[v] + 1;
        dfs(to, v);                // Плевать на условия, ибо дерево
    }
}
